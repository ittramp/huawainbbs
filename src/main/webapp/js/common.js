...
var Util = {
        ...
        $.ajax({
            ...
            success: function (result, textStatus) {
                ...
                if ($.ua.device.type && $.ua.device.type === 'mobile') {
                    if (0 < count) {
                        $("#aNotifications").removeClass("no-msg").addClass("msg").attr('href', 'javascript:void(0)')
                            .find(".msg_count_wrapper").text(count);
                        ...
                        $("#aNotifications").removeClass("msg").addClass("no-msg")
                ...
                // browser
                if (0 < count) {
                    $("#aNotifications").removeClass("no-msg tooltipped tooltipped-w")
                        .addClass("msg").attr('href', 'javascript:void(0)')
                        .find(".msg_count_wrapper").text(count);
                    if (0 === result.userNotifyStatus && window.localStorage.hadNotificate !== count.toString() && isSendMsg) {
                        Util.notifyMsg(count);
                        window.localStorage.hadNotificate = count;
                    ...
                    $("#aNotifications").removeClass("msg").addClass("no-msg tooltipped tooltipped-w")
                        .attr('href', Label.servePath + '/notifications')
                        .find(".msg_count_wrapper").text(count);
                }
            }
    ...
    _initNav: function () {
        ...
        $('.nav .avatar-small').parent().click(function () {
            ...
            var personListPanelObj = $('#personListPanel');
            var targetLeft = position.left - personListPanelObj.width();
            if(targetLeft < 0) {
                targetLeft = 0;
            }
            //fixme: left的值可能需要微调
            $('#personListPanel').offset({
