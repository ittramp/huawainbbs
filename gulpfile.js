...
function minCodemirrorCSS() {
    // min css
    return gulp.src('./src/main/webapp/js/lib/editor/codemirror.css').pipe(cleanCSS()).pipe(concat('codemirror.min.css')).pipe(gulp.dest('./src/main/webapp/js/lib/editor/'))
}
...
function minArticleCSS() {
    ...
    return gulp.src([
        ...
        './src/main/webapp/js/lib/diff2html/diff2html.min.css']).pipe(cleanCSS()).pipe(concat('article.min.css')).pipe(gulp.dest('./src/main/webapp/js/lib/compress/'))
}

function minJS() {
    // min js
    return gulp.src('./src/main/webapp/js/*.js').pipe(uglify()).pipe(rename({suffix: '.min'})).pipe(gulp.dest('./src/main/webapp/js/'))
}
...
function minCodemirrorJS() {
    ...
    return gulp.src(jsCodemirror).pipe(uglify({output: {ascii_only: true}})) // https://github.com/b3log/symphony/issues/765
        .pipe(concat('codemirror.min.js')).pipe(gulp.dest('./src/main/webapp/js/lib/editor/'))
}
